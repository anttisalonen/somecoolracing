...
int run_game(const char* carname, const char* trackname)
{
	Game g;
	g.run(carname, trackname);
	return 0;
}
...
int main(int argc, char** argv)
{
	...
	for(int i = 0; i < argc; i++) {
		if(!strcmp(argv[i], "-t")) {
			...
			i++;
			if(i == argc) {
				std::cerr << "--track requires an argument.\n";
				return 1;
			}
			trackname = argv[i];
		}
